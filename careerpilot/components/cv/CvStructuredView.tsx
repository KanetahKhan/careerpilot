...
function parseProject(content: string): {
  ...
  const githubUrl = lines.find(l => /^Github:/i.test(l))?.replace(/^Github:\s*/i, "") || "";
  const liveUrl = lines.find(l => /^Live:/i.test(l))?.replace(/^Live:\s*/i, "") || "";
  const desc = lines.filter(l => !/^(Name|Tech|Github|Live):\s*/i.test(l)).join("\n");
  return { name: name || undefined, tech: tech.length > 0 ? tech : undefined, description: desc || undefined, githubUrl: githubUrl || undefined, liveUrl: liveUrl || undefined };
}
            ...
            (groupedChunks.projects || []).map((chunk) => {
              ...
              return (
                <div key={chunk.position} className="panel p-5">
                  ...
                  {(parsed.githubUrl || parsed.liveUrl) && (
