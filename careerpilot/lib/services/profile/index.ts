 ...
 *
 * Responsibility: turn an uploaded or builder-created CV into section-tagged,
 * embedded chunks and retrieve the most relevant chunks for a query. This is
 * the single source of truth every other service grounds against — nothing
 * about the user is invented.
 *
 * Public API:
 *   Ingestion (file) · ingestCv(userId, buffer, fileName) → IngestResult
 *   Ingestion (shared)· ingestSections(userId, sections, fileName?) → IngestResult
 *              · extractText(buffer, fileName) → raw text
 ...
 *
 * Inputs:  PDF/DOCX/text buffer or structured sections (ingestion);
 *          userId + query (retrieval).
